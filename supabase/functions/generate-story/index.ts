...
### 📝 חריגה מותרת (Fallback)
אם מילה מורכבת **הכרחית** לסיפור, **חובה** להוסיף הסבר פשוט בסוגריים:
...
### גילאי 0-2
- **אורך:** קצר מאוד, עד 100 מילים
- משפטים קצרצרים (3-5 מילים)
- מילים פשוטות וחזרות
- מיקוד חושי ומרגיע
...
### גילאי 7-8
- **אורך:** ארוך יותר עם אוצר מילים עשיר יותר (עם הסברים!)
- עלילה מורכבת עם רגשות פנימיים
- מצבים חברתיים מורכבים יותר
- אם יש מילים מורכבות - **חובה הסבר בסוגריים**
...
## ✍️ מבנה הסיפור - 5 עמודים בלבד!

כל עמוד מכיל 2-3 משפטים קצרים וברורים.

מבנה העלילה (Social Story Format):
1. **עמוד 1:** פתיחה - הצגת הדמות והמצב
2. **עמוד 2:** האתגר - תיאור המצב, הכרה ברגשות
3. **עמוד 3:** כלים - הדמות מגלה דרך להתמודד
4. **עמוד 4:** הצלחה - הדמות משתמשת בכלים ומצליחה
5. **עמוד 5:** מנטרה מעצימה - משפט קצר וקליט לזכור
...
`
    ...
    const getAgeLengthInstruction = (age: string) => {
      if (age === "0-2") {
        return `- גיל 0-2: סיפור קצר מאוד (עד 100 מילים סה"כ)
...
- כל עמוד: משפט אחד בלבד!`;
        ...
        return `- גיל 3-6: סיפור באורך בינוני
- משפטים קצרים וברורים (2-3 משפטים בעמוד)
- סיבה ותוצאה ברורות
- אינטראקציות חברתיות פשוטות`;
      } else {
        return `- גיל 7-8: סיפור ארוך ומורכב יותר (מותאם לילדים לומדי קריאה)
...
- 3-4 משפטים בעמוד`;
    ...
    const ageLengthInstruction = getAgeLengthInstruction(ageRange);
...
**צור בדיוק 5 עמודים!** לא יותר, לא פחות.

**הנחיות אורך וסגנון לפי גיל ${ageRange}:**
${ageLengthInstruction}
