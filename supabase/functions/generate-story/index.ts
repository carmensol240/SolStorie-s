...
## 🌍 עולם SolStories - דמויות משנה (Supporting Characters)
**כאשר העלילה דורשת חברים או דמויות תומכות מעבר לגיבור הראשי, אין ליצור דמויות חדשות אקראיות!**
השתמש אך ורק ב"חברים של סול" - הקאסט הקבוע של עולם SolStories:
1. **ילדת הטבע (The Nature Girl)** - לובשת שמלה ירוקה, שיער חום קצר וישר בתספורת בוב (bob cut). אוהבת טבע, בעלי חיים וחרקים (נראית לעיתים קרובות עם פרפרים).
2. **הילד החכם (The Smart Boy)** - לובש אפודת סוודר כחולה ומשקפיים, שיער שחור. אוהב לקרוא ותמיד שואל "למה?". סקרן וחושב לפני שהוא פועל.
3. **הילדה הספורטיבית (The Sporty Girl)** - לובשת חליפת ספורט סגולה-צהובה, שיער מתולתל/אפרו גדול ושופע. אוהבת כדורגל ופעולה, נמרצת ואמיצה.
4. **הילד הצייר (The Artist Boy)** - לובש סרבל ג'ינס מותז בצבע, שיער חום מבולגן. אוהב לצייר וליצור, יצירתי וחולמני.
5. **סול (המארחת/מרכז העולם)** - לובשת שמלה צהובה בהירה, שיער חום ארוך וישר עד גלי קלות אסוף בקוקו גבוה. מייצגת את רוח ההרפתקה (מצפן). סול היא דמות הרקע של העולם - היא יכולה להופיע כדמות תומכת אך הגיבור הראשי הוא תמיד הילד של המשתמש.
- **שלב 1-2 מהחברים האלו לפי הצורך** - בחר את הדמויות המתאימות ביותר לנושא הסיפור.
