...
// Deterministic seed per story so that any seed-capable model (Flux Schnell fallback)
// renders the same character across every page of the SAME story.
...
// Wait (bounded) until page 1 of the story has a finished illustration, then return it
// as a public URL. In distributed mode every page is generated by a separate concurrent
// invocation, so without this wait pages 2+ would start before page 1 exists and would
// be generated with NO character reference — which is what made the character drift.
async function waitForPageOneIllustration(
  ...
  while (Date.now() < deadline) {
    ...
    const { data } = await supabase
      ...
      .eq("page_number", 1)
      .maybeSingle();
    const url = buildPublicIllustrationUrl(data?.illustration_url || null);
    if (url) {
  ...
  console.warn(`⚠️ [ref-wait] story=${storyId} page-1 illustration NOT ready after ${maxWaitMs}ms — continuing without character reference`);
  return null;
}

// Unified log line for every image-generation API call (see also generate-cover / retry-illustration).
export function logImageGenCall(params: {
  storyId: string;
  page: number | string;
  api: string;
  model: string;
