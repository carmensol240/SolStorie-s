...
const GiftCard = () => {
  ...
  const handleShareWhatsApp = () => {
    ...
    const message = `מתנה קסומה מחכה לכם! ${sender} שלחה לכם חבילה של ${selectedPkg.stories} סיפורים אישיים במתנה, שבהם סול (או שם הילד) הופכת לגיבורה של הרפתקאות מרגשות. איך מממשים? נכנסים ונרשמים בקלות בכתובת https://www.soulstory.co.il (או מתחברים), ומזינים את קוד הקופון האישי שלכם: ${generatedCode}! קריאה מהנה ומרגשת! ❤️`;
